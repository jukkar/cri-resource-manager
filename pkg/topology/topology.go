...
// SetSysfsRoot sets the sysfs root directory to use.
...
func getDevicesFromVirtual(realDevPath string) (devs []string, err error) {
	...
	switch dir {
	case "vfio/":
		iommuGroup := filepath.Join(sysfsRoot, "/sys/kernel/iommu_groups", file, "devices")
...
func NewTopologyHints(devPath string) (hints Hints, err error) {
	...
	for p := realDevPath; strings.HasPrefix(p, sysfsRoot+"/sys/devices/"); p = filepath.Dir(p) {
