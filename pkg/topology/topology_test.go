...
func TestGetDevicesFromVirtual(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			output:      []string{sysfsRoot + "/sys/devices/pci0000:00/0000:00:02.0"},
			expectedErr: false,
		},
		{
			name:        "misc",
...
func TestNewTopologyHints(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name:  "pci card1",
			input: sysfsRoot + "/sys/devices/pci0000:00/0000:00:02.0/drm/card1",
			output: Hints{
				sysfsRoot + "/sys/devices/pci0000:00/0000:00:02.0": Hint{
					Provider: sysfsRoot + "/sys/devices/pci0000:00/0000:00:02.0",
					CPUs:     "0-7",
					NUMAs:    "",
					Sockets:  ""},
			},
